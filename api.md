...
Internal Method. Verifies if a key needs to be fetched from the database. If persistent enmap and autoFetch is on, retrieves the key.

**Kind**: instance method of [`Enmap`](api.md#Enmap)

| Param | Type | Description |
| :--- | :--- | :--- |
| key | `string` \| `number` | The key to check or fetch. |

### enmap.\_parseData\(data\) ⇒ `*`

Internal Method. Parses JSON data. Reserved for future use \(logical checking\)
...
| Param | Type | Description |
| :--- | :--- | :--- |
| pool | `Map` | In order to set data to the Enmap, one must be provided. |
